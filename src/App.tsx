            ...
            if (user) {
                ...
                const attemptSignIn = async () => {
                    try {
                        ...
                        // 設置通用的 Firebase 錯誤信息
                        setFirebaseError(
                            '認證配置錯誤 (auth/configuration-not-found)。請檢查您的 Firebase 專案中是否啟用了 Authentication 服務。',
                        );
            ...
            const unsubscribe = onSnapshot(
                ...
                (error) => {
                    ...
                    if (error.code === 'permission-denied') {
                        setFirebaseError(
                            'Firestore 權限不足 (Missing or insufficient permissions)。請檢查您的 **Firestore 安全規則**，確保允許對 `/artifacts/{appId}/public/data/restaurants` 進行讀寫。',
                        );
                    } else {
    ...
    const handleAddRestaurant = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if ((error as any).code === 'permission-denied') {
                setFirebaseError(
                    'Firestore 權限不足 (Missing or insufficient permissions)。請檢查您的 **Firestore 安全規則**。',
                );
            } else {
                    ...
                    <div className="mb-8 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg shadow-md">
                        ...
                        {firebaseError.includes('權限不足') && (
                            <p className="text-sm mt-2 font-medium">
                                請到 Firebase Console，檢查 **Firestore Database Rules**
                                是否允許已認證用戶讀寫公共路徑。
                            </p>
                        )}
                    </div>
